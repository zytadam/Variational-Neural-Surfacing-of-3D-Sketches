...
class DPSR(nn.Module):
    ...
    def forward(self, V_f, N_f, V_c, avg=False, normalize=False, trivial=True):
        """
        :param V: (batch, nv, 2 or 3) tensor for point cloud coordinates
        :param N: (batch, nv, 2 or 3) tensor for point normals
        :return phi: (batch, res, res, ...) tensor of output indicator function field
        """
        assert(V_f.shape == N_f.shape) # [b, nv, ndims]
        V = V_f
        N = N_f
        ...
        if not avg:
            ...
            ras_p = point_weighted_avg_rasterize(V, N, self.res)


        if normalize:
            # avg_norm = torch.mean(torch.norm(ras_p, dim=1, keepdim=True), dim=(2,3,4), keepdim=True)
            norm = torch.norm(ras_p, dim=1, keepdim=True)  # [b, 1, dim0, dim1, dim2]
            # ras_p_n = torch.nn.functional.normalize(ras_p, dim=1)  # normalize ras_p to have unit length
            # ratio = torch.nn.functional.sigmoid(norm**2)
            # ras_p = (1-ratio) * ras_p + ratio * ras_p_n
            # ras_p = ras_p * avg_norm
            if not trivial:
                ras_p = ras_p / torch.max(torch.ones_like(norm), norm)  # [b, 1, dim0, dim1, dim2]
            else:
                ras_p = ras_p / (norm + 1e-6)
        
        self.ras_p = ras_p.detach().cpu().numpy()
        ...
        if self.shift or self.scale:
            # ensure values at curve points are zero
            fv = grid_interp(phi.unsqueeze(-1), V_c, batched=True).squeeze(-1) # [b, nv]
            if self.shift: # offset points to have mean of 0
                offset = torch.mean(fv, dim=-1)  # [b,] 
                phi -= offset.view(*tuple([-1] + [1] * self.dim))
        ...
        phi_c = grid_interp(phi.unsqueeze(-1), V_c, batched=True).squeeze(-1)
...
if __name__ == '__main__':
    ...
    res = (32, 32, 32)
    sig = 10
    dpsr = DPSR(res, sig).cuda()
    V = torch.rand(1, 15000, 3).cuda()
    N = torch.rand(1, 15000, 3).cuda()
    phi = dpsr(V, N)
    print(phi.shape)