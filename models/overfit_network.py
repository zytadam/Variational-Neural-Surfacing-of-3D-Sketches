...
class Network(nn.Module):
    ...
    def forward(self, non_mnfld_pnts, mnfld_pnts=None, curve_points=None, sub_curve_points=None, only_nonmnfld=False, latent=None):
        ...
        if curve_points is not None:
            curve_points_pred = self.decoder(curve_points, mods)
        else:
            curve_points_pred = None

        if sub_curve_points is not None:
            sub_curve_points_pred = self.decoder(sub_curve_points, mods)
        else:
            sub_curve_points_pred = None

        return {"manifold_pnts_pred": manifold_pnts_pred,
                "nonmanifold_pnts_pred": nonmanifold_pnts_pred,
                'curve_points_pred': curve_points_pred,
                'sub_curve_points_pred': sub_curve_points_pred,
