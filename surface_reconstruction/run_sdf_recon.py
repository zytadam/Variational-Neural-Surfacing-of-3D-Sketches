...
if __name__ == '__main__':
    ...
    n_samples = 1000  # iterations number
    batch_size = 10
    grid_res = 256  # grid resolution
    ...
    # loss_weights = (1e4, 7e3, 6e2, 7e3, 0, 1000)
    # loss_weights = (1e3, 7e3, 1e3, 1e3, 0, 100)
    morse_type = 'l1'
    ...
    for f in sorted(os.listdir(data_path)):
        if os.path.splitext(f)[1] in ['.ply', '.obj']:  # only accept .obj and .ply files
            files.append(f)
    used = list()
        ...
        if len(device_ID) != 0 and (
            ...
            def sub_thread():
                ...
                os.system(f'CUDA_VISIBLE_DEVICES={id} python train_surface_reconstruction.py \
                          --logdir {logdir} --data_path {os.path.join(data_path, f)} --n_samples {n_samples} --n_points {n_points} --grid_res {grid_res} --grid_size {grid_size}\
