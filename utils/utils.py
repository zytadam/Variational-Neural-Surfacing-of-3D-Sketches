...
def check_nan(tensor, name="Tensor"):
    nan_inf_mask = torch.isnan(tensor) | torch.isinf(tensor)
    nan_inf_per_subtensor = nan_inf_mask.view(tensor.shape[0], tensor.shape[1], -1).any(dim=-1)
    indices = torch.nonzero(nan_inf_per_subtensor, as_tuple=False)
    for b, n in indices:
        print(f"{name} at index ({b.item()}, {n.item()}) contains NaN or Inf:")
        print(tensor[b, n])
    return indices

def print_nan(tensor, indices, name="Tensor"):
    for b, n in indices:
        print(f"{name} at index ({b.item()}, {n.item()}):")
        print(tensor[b, n])
